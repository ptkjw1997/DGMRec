...
class SLMRec(GeneralRecommender):
    ...
    def compute(self):
        ...
        if self.config["dataset"] == "kwai":
            ...
            if self.a_feat is not None:
                user = self.embedding_user_after_GCN(self.mm_fusion([self.i_emb_u, self.v_emb_u, self.t_emb_u, self.a_emb_u]))
                item = self.embedding_item_after_GCN(self.mm_fusion([self.i_emb_i, self.v_emb_i, self.t_emb_i, self.a_emb_i]))
            else:
    ...
    def fac(self, idx):
        if self.a_feat is None :
            if self.missing_modal :
                t_index = np.isin(idx.detach().cpu().numpy(), self.missing_items_t)
                v_index = np.isin(idx.detach().cpu().numpy(), self.missing_items_v)
                tv_index = np.isin(idx.detach().cpu().numpy(), np.union1d(self.missing_items_t, self.missing_items_v))
            else :
                t_index = idx.detach().cpu().numpy()
                v_index = idx.detach().cpu().numpy()
                tv_index = idx.detach().cpu().numpy()
