...
class LATTICE(GeneralRecommender):
        ...
        if self.a_feat is not None:
            ...
            if self.v_feat is not None:
                ...
                if self.new_items :
                    ...
                    image_adj = build_knn_neighbourhood(image_adj, topk=self.knn_k)
                    image_adj = compute_normalized_laplacian(image_adj)
                    self.image_original_adj_newitems = image_adj.cuda().to_sparse_coo()
                    torch.cuda.empty_cache()
                pass
    ...
    def preprocess_missing_modal(self, config) :
        ...
        if config['missing_imputation'] == 0 :
            ...
            if self.a_feat is not None :
                # 3-modality historical behavior preserved: mean over np.arange(n_items) instead of old_items_set
                non_missing_item_t = np.setdiff1d(np.arange(self.n_items), self.missing_items_t)
                non_missing_item_v = np.setdiff1d(np.arange(self.n_items), self.missing_items_v)
                non_missing_item_a = np.setdiff1d(np.arange(self.n_items), self.missing_items_a)
            ...
            if self.a_feat is not None :
                # 3-modality historical behavior preserved: tiktok tree has no imputation==2 branch
    ...
    def forward(self, adj, build_item_graph=False):
        ...
        if build_item_graph:
            ...
            if self.a_feat is not None:
                ...
                self.text_adj = build_sim(text_feats)
                if self.new_items :
                    self.text_adj[self.new_items_set, :] = 0.0
                    self.text_adj[:, self.new_items_set] = 0.0
                self.text_adj = build_knn_neighbourhood(self.text_adj, topk=self.knn_k)
                torch.cuda.empty_cache()
                ...
                self.org_adj = self.lambda_coeff * (weight[0] * image_org_adj + weight[1] * text_org_adj)
            ...
            if self.a_feat is None:
                self.org_adj = self.org_adj.detach()
    ...
    def forward_inference(self, adj, build_item_graph=False):
        if self.v_feat is not None:
            image_feats = self.image_trs(self.image_embedding.weight)
        if self.t_feat is not None:
            text_feats = self.text_trs(self.text_embedding.weight)
        if build_item_graph:
            ...
            if self.item_adj is not None:
                del self.item_adj
            self.item_adj = (1 - self.lambda_coeff) * learned_adj
            self.org_adj = self.lambda_coeff * (weight[0] * self.image_original_adj + weight[1] * self.text_original_adj)
        else:
            self.item_adj = self.item_adj.detach()
            self.org_adj =self.org_adj.detach()
