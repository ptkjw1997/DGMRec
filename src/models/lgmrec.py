...
class LGMRec(GeneralRecommender):
    ...
    def preprocess_missing_modal(self, config) :
        ...
        if config['missing_imputation'] == 0 :
            ...
            if self.a_feat is not None :
                # 3-modality historical behavior preserved: no NN imputation in tiktok tree (pass)
                pass
            else :
                non_missing_item_t = np.setdiff1d(self.old_items_set, self.missing_items_t)
                non_missing_item_v = np.setdiff1d(self.old_items_set, self.missing_items_v)

                adj = torch.sparse.mm(self.adj.t(), self.adj).to_dense()
                adj = (adj > 0).float()
                adj.fill_diagonal_(0)
                sumArr = (adj > 0).sum(axis = 1)
                sumArr = 1.0 / (sumArr + 1e-7)

                image_NN = torch.einsum("ij, i -> ij", torch.sparse.mm(adj, self.v_feat), sumArr)
                text_NN = torch.einsum("ij, i -> ij", torch.sparse.mm(adj, self.t_feat), sumArr)

                self.v_feat[self.missing_items_v] = image_NN[self.missing_items_v]
                self.t_feat[self.missing_items_t] = text_NN[self.missing_items_t]
        else :
            if self.a_feat is not None :
    ...
    def forward(self, image_embedding = None, text_embedding = None, audio_embedding = None):
        ...
        if self.t_feat is not None:
            it_hyper = torch.mm(text_embedding, self.t_hyper)
        ...
        if self.a_feat is not None:
            ia_hyper = torch.mm(audio_embedding, self.a_hyper)
            ua_hyper = torch.mm(self.adj, ia_hyper)
        ...
        if self.v_feat is not None and self.t_feat is not None:
            ...
            if self.a_feat is not None:
                ua_hyper_embs, ia_hyper_embs = self.hgnnLayer(self.drop(ia_hyper), self.drop(ua_hyper), cge_embs[self.n_users:])
            av_hyper_embs = torch.concat([uv_hyper_embs, iv_hyper_embs], dim=0)
            at_hyper_embs = torch.concat([ut_hyper_embs, it_hyper_embs], dim=0)
            if self.a_feat is not None:
                aa_hyper_embs = torch.concat([ua_hyper_embs, ia_hyper_embs], dim=0)
        ...
        if self.a_feat is not None:
            return u_embs, i_embs, [uv_hyper_embs, iv_hyper_embs, ut_hyper_embs, it_hyper_embs, ua_hyper_embs, ia_hyper_embs]
    ...
    def calculate_loss(self, interaction):
        ...
        if self.a_feat is not None:
            ...
            batch_hcl_loss += self.ssl_triple_loss(ut_embs[users], ua_embs[users], ua_embs) + self.ssl_triple_loss(it_embs[pos_items], ia_embs[pos_items], ia_embs[self.complete_items])
        else:
            [uv_embs, iv_embs, ut_embs, it_embs] = hyper_embeddings
            batch_hcl_loss = self.ssl_triple_loss(uv_embs[users], ut_embs[users], ut_embs) + self.ssl_triple_loss(iv_embs[tv_index], it_embs[tv_index], it_embs[self.complete_items])
